...
public class ZipResourceFile {
    ...
    private HashMap<String, ZipResourceFile.ZipEntryRO> mHashMap = new HashMap<String, ZipResourceFile.ZipEntryRO>();
    ...
    ZipResourceFile.ZipEntryRO[] getEntriesAt(String path) {
        Vector<ZipResourceFile.ZipEntryRO> zev = new Vector<ZipResourceFile.ZipEntryRO>();
        Collection<ZipResourceFile.ZipEntryRO> values = mHashMap.values();
        if (null == path)
            path = "";
        int length = path.length();
        for (ZipResourceFile.ZipEntryRO ze : values) {
        ...
        ZipResourceFile.ZipEntryRO[] entries = new ZipResourceFile.ZipEntryRO[zev.size()];
        return zev.toArray(entries);
    }

    public ZipResourceFile.ZipEntryRO[] getAllEntries() {
        Collection<ZipResourceFile.ZipEntryRO> values = mHashMap.values();
        return values.toArray(new ZipResourceFile.ZipEntryRO[values.size()]);
    }

    public AssetFileDescriptor getAssetFileDescriptor(String assetPath) {
        ZipResourceFile.ZipEntryRO entry = mHashMap.get(assetPath);
        if (null != entry) {
    ...
    private void parseCentralDirectory(boolean setOffsets) throws IOException {
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            ZipResourceFile.ZipEntryRO ze = new ZipResourceFile.ZipEntryRO(str);
